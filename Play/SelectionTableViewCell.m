...
const float CUES_MARGIN = 10.0f;
const float CUES_WIDTH = 50.0f;
...
- (void)layoutSubviews {
    ...
    _crossImage.frame = CGRectMake(self.bounds.size.width + CUES_MARGIN, 0, CUES_WIDTH, self.bounds.size.height);
...
-(void)handlePan:(UIPanGestureRecognizer*)recognizer {
    ...
    if(recognizer.state == UIGestureRecognizerStateChanged){
        ...
        if (translation.x < 0) {
            ...
            if(-translation.x > CUES_WIDTH + (2 * CUES_MARGIN)) {
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded){
        ...
        if(_deleteOnDragRelease){
            [self.delegate playlistItemDeleted:self.playlistItem];
        }
    }
}
